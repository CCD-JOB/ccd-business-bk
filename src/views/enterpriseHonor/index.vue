<template>
	<div class="enterpriseHonorWrapper">
		<h1>荣誉证书上传</h1>
		<a-card :bordered="false">
			<a-form layout="horizontal"
				:form="form">
				<a-form-item label="证书全称："
					:label-col="{ span: 2 }"
					:wrapper-col="{ span: 12 }">
					<a-input v-decorator="['honorName',
          {rules: [{ required: true, message: '请输入荣誉证书完整名称!', trigger: 'change' }]}
        ]"
						placeholder="请输入荣誉证书完整名称！"></a-input>
				</a-form-item>
				<a-form-item :label-col="{ span: 2 }"
					:wrapper-col="{ span: 12 }">
					<span slot='label'><em style="color:red;">*</em> 上传图片</span>
					<prev-img-upload uploadTitle="上传荣誉证书"
...
export default {
  ...
  data () {
    return {
      ...
      loadData: () => {
        return showCompanyHonors(this.queryParam).then(res => {
  ...
  methods: {
    ...
    handlePreviewImg (file) {
      this.modalInfo = {
        ...
        title: file.honorName || '查看大图',
        src: file.winPath || file.honorUrl.split(';')[0],
        previewVisible: true
      }
    },
    beforeUpload (file) {
    ...
    async handleDeleteUpload (item) {
      try {
        let res = await removeCompanyHonorPic({
          fileUrl: item.fileUrl || item.honorUrl.split(';')[1],
          id: item.id
        })
        if (res.code === 200) {
          this.uploadInfo = {
    ...
    async handleSubmit () {
      ...
      const formData = new FormData()
      const honorName = getFieldsValue(['honorName']).honorName
      if (!fileList.length) {
        this.$notification.error({
          message: '请上传证书！'
        })
      ...
      validateFields(['honorName'], async err => {
        if (!err) {
          try {
            this.uploading = true
            let param = `?honorName=${honorName}&companyUrl=${
              this.companyUrl
